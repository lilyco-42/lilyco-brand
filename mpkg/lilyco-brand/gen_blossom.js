// lilyco 标识生成器 v3 —— 逐帧复刻 OpenAI Blossom 构造
// 几何基线：simple-icons v15 `openai.svg` 的 24×24 官方路径。
// v1/v2 的参数化 6 钩（RH/C1/C2）废弃：内旋质感始终差一口气 —— 根因是自设的
// 「弧 + 内旋螺旋」在拓扑上就不是真花结；真花结是 6 条缎带互扣，以 8 个子路径
// 的 compound path 表达（外编织轮廓 + 6 片内瓣边界 + 中心小六边形留空）。
const fs = require('fs');
...
`<svg xmlns="http://www.w3.org/2000/svg" viewBox="-120 -120 240 240">
  ...
  <circle cx="0" cy="0" r="30" fill="none" stroke="#7E92C6" stroke-width="0.75" stroke-dasharray="4 4"/>
</svg>
`);

// 统计自检
const subs = (D.match(/[mM]/g) || []).length;
console.log('blossom path: ' + D.length + ' chars, ' + subs + ' subpaths, scale ' + S);
console.log('wrote artifacts/lilyco-mark.svg + lilyco-mark-construction.svg');
